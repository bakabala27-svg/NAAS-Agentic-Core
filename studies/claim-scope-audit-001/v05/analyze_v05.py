...
from cap_decoder import N_MIN, cap_pct, consistent_n, decode, identifiability_census

HERE = "studies/claim-scope-audit-001/v05/"
EFFORTS = ["max", "xhigh", "high", "medium", "low", "none"]

# ---------- load ----------
with open(HERE + "cells_public_v05.csv") as _fh:
...
for h in ("standard", "adapter"):
    for eff in EFFORTS:
        ...
        E = math.sqrt(r / 100)
        say(
            f"  {h:8s} {eff:6s}: RHAE={r:6.2f}%  E=sqrt(RHAE)={E:.6f}  actions/human=1/E={1 / E:.4f}x  cost=${COST[h][eff]:,}"
        )
...
for e1 in EFFORTS:
    for e2 in EFFORTS:
        if not (
            SP["adapter"][e1] > SP["standard"][e2] and COST["adapter"][e1] < COST["standard"][e2]
        ):
            viol += 1
        min_cost_margin = min(min_cost_margin, COST["standard"][e2] - COST["adapter"][e1])
say(
    f"  violations: {viol}/36; cost margin = cheapest standard - priciest adapter = "
...
for label, r in (
    ("99.95 (adapter high)", 99.95),
    ("62.71 (std max)", 62.71),
    ("17.45 (std low)", 17.45),
    ("2.11 (Grok 4.6, relay)", 2.11),
):
    E = math.sqrt(r / 100)
    say(
        f"  at {label:26s}: 1 RHAE pt = {100 / (200 * E):.3f}% actions-efficiency; amp=2E={2 * E:.3f}x"
    )
gap_hi = SP["adapter"]["high"] - SP["adapter"]["max"]
say(
    f"  99.95 vs 98.55: score gap {gap_hi:.2f} pts -> action-efficiency gap "
    f"{100 * (math.sqrt(0.9995) - math.sqrt(0.9855)):.3f}% (=divide by ~{gap_hi / (100 * (math.sqrt(0.9995) - math.sqrt(0.9855))):.2f})"
)

# ---------- 5. N4 effort-dial economics ----------
say("\n== 5. effort-dial economics ==")
d_eff = 100 * (math.sqrt(SP["adapter"]["high"] / 100) - math.sqrt(SP["adapter"]["max"] / 100))
d_cost = COST["adapter"]["high"] - COST["adapter"]["max"]
say(
    f"  adapter max->high: +{SP['adapter']['high'] - SP['adapter']['max']:.2f} RHAE pts for +${d_cost:,} "
    f"= ${d_cost / (SP['adapter']['high'] - SP['adapter']['max']):,.0f}/pt = ${d_cost / d_eff:,.0f} per 1% action-efficiency"
)
span = 100 * (math.sqrt(SP["adapter"]["high"] / 100) - math.sqrt(SP["adapter"]["none"] / 100))
say(
    f"  adapter none->high: +{SP['adapter']['high'] - SP['adapter']['none']:.2f} RHAE pts = +{span:.2f}% action-efficiency; "
...
TOL = 0.06
nontrivial = [
    (e, eff) for e in envs_std for eff in EFFORTS if 0.0 < cells[("standard", e, eff)] < 100.0
]
say(
    f"  non-trivial standard cells (0 < score < 100): {len(nontrivial)}  "
...
for env in sorted(cons):
    ...
    say(
        f"    {env}: n-candidates -> "
        + "; ".join(f"n={n}: {sorted({k for _, k in v})} ({len(v)} cells)" for n, v in ns.items())
    )
...
tail = sum(
    math.comb(n_nt, i) * p_chance**i * (1 - p_chance) ** (n_nt - i) for i in range(k_obs, n_nt + 1)
)
say(
    f"  published-grid bins covered by any cap value: {len(grid_hits)}/999 -> p(match by chance)={p_chance:.4f}"
)
say(
    f"  expected chance matches in {n_nt} cells: {n_nt * p_chance:.1f}; observed {k_obs}; exact binomial tail p = {tail:.3e}"
)
...
say(
    f"  every human score <= cap(8,k): {ok}  (cap(8,2)={cap_pct(8, 2):.2f}, cap(8,6)={cap_pct(8, 6):.2f})"
)
say(
    f"  decoder on our RE86 cells: {sorted({(n, k) for (e, eff, s, h) in all_matches if e == 'RE86' for (n, k, c) in h})}"
)
say(
    f"  Astra RE86 max/xhigh/high = {cells[('standard', 'RE86', 'max')]} = cap(8,7)={cap_pct(8, 7):.2f} -> k=7 of n=8; KILL GATE: report says Lvl/8 -> SURVIVES"
)

# ls20 out-of-sample: report Figure 8 shows 7 levels
say(
    f"  ls20 decoder hits: {sorted({(n, k) for (e, eff, s, h) in all_matches if e == 'LS20' for (n, k, c) in h})} "
...
pub_drop = statistics.mean([cells[("standard", e, "none")] for e in envs_std]) - statistics.mean(
    [cells[("standard", e, "low")] for e in envs_std]
)
say(
    f"  public drop none->low = {pub_drop:.2f} pts; semi-private drop = {SP['standard']['none'] - SP['standard']['low']:.2f} pts; "
...
collapse = [
    e
    for e in worse
    if decode(cells[("standard", e, "low")], tol=TOL)
    and not decode(cells[("standard", e, "none")], tol=TOL)
]
say(
    f"  of the {len(worse)}: collapse-to-cap at low (completion stop): {[e for e in worse if decode(cells[('standard', e, 'low')], tol=TOL)]}"
)
...
for eff in EFFORTS:
    ...
    say(
        f"  {eff:6s}: public={pub_means[eff]:6.2f}  semi-private={SP['standard'][eff]:6.2f}  inflation={infl:+6.2f}"
    )
...
say(
    f"  cells at exactly 100.0: {len(a100)}/150 (incl. {sum(1 for k in a100 if prov[('adapter', *k)] == 'infer-saturation')} inferred)"
)
...
say(
    f"  adapter 'none' public mean = {none_mean:.3f}  [blog-era figure 97.11 -> {'MATCH' if abs(none_mean - 97.11) < 0.01 else 'MISMATCH'}]"
)
for eff in ("max", "xhigh", "medium"):
    vals = [cells[("adapter", e, eff)] for e in envs_std]
    say(
        f"  adapter {eff}: all-100 = {all(v == 100.0 for v in vals)} (direct-only: {all(v == 100.0 for e in envs_std for v in [cells[('adapter', e, eff)]] if prov[('adapter', e, eff)] == 'direct')})"
    )
...
def spearman(x, y):
    ...
    n = len(x)
    d2 = sum((rx[a] - ry[b]) ** 2 for a, b in zip(x, y, strict=False))
...
say("  blog's own sentence: 'Higher reasoning levels generally cost LESS' [P]")
...
say(
    f"  total delta = {sum(d for d, _ in deltas):.1f} pts over 25 envs; mean = {statistics.mean(d for d, _ in deltas):.2f}"
)
...
say(
    f"  score-space inflation vs action-space: {(viral - math.sqrt(viral)) / math.sqrt(viral) * 100:.1f}%"
)
say(
    f"  vs April-2026 official frontier top (Opus 4.6 Max = 0.50%, Table 2): "
    f"std-max {SP['standard']['max'] / 0.50:.1f}x score / {math.sqrt(SP['standard']['max'] / 0.50):.1f}x actions"
)
say(
    f"  human wage anchor: 55 envs x $12.78 = ${55 * HUMAN_FEE:,.2f}; cheapest adapter run = "
...
say(
    f"  re86 human RHAE distribution (Fig.4): mean={statistics.mean(fig4):.1f}% median={statistics.median(fig4)}% "
...
say(
    f"  cells below the k=1 floor across all 300: {len(viol_floor)} {viol_floor}  (n=12 worst case: {100 * 0.04 / (12 * 13 / 2):.3f}%)"
)
...
say(
    f"  Pareto min-adapter {min(v02[e][1] for e in v02)} > max-standard {max(v02[e][0] for e in v02)}; "
...
with open(HERE + "derived_v05.txt", "w") as _out_fh:
    _out_fh.write("\n".join(out))
