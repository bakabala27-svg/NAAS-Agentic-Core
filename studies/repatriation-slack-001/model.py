...
RAILS = [
    ...
    {
        "id": "R4",
        "name": "MoR semi-monthly (Lemon Squeezy pattern)",
        "terms": 0,
        "hold": 13,
        "cadence": 15,
        "transit": 5,
    ...
    {
        "id": "R5",
        "name": "MoR monthly net-30 (Paddle pattern)",
        "terms": 0,
        "hold": 30,
        "cadence": 30,
        "transit": 5,
...
RESERVES = [
    {"id": "RS0", "name": "no reserve", "pct": 0.00, "window": 0},
    {"id": "RS1", "name": "25% / 30-day rolling", "pct": 0.25, "window": 30},
    {"id": "RS2", "name": "25% / 90-day rolling", "pct": 0.25, "window": 90},
    {"id": "RS3", "name": "10% / 180-day rolling", "pct": 0.10, "window": 180},
]

# Two admissible readings of D0 for a 12-month prepaid subscription.
INTERPRETATIONS = [
    {"id": "I-A", "name": "D0 = invoice / access granted (day 0)", "offset": 0},
    {"id": "I-B", "name": "D0 = end of subscription period (day 365)", "offset": 365},
]
...
def exposed_share(row: dict) -> float:
    ...
    return (1 - row["reserve_pct"]) * (1 if row["slack_main"] < 0 else 0) + row["reserve_pct"] * (
        1 if row["slack_reserved"] < 0 else 0
    )
...
def main() -> None:
    ...
    print(line)
    print(f"RSM v0.1 | ceiling = {CEILING_STD} days | reading I-A (D0 = day 0)")
    print(line)
    print(
        f"{'rail':<40}{'reserve':<24}{'main d':>8}{'slack':>8}{'res d':>8}{'slack':>8}{'verdict':>10}"
    )
    print("-" * 104)
    for r in rows:
        ...
        worst = (
            v_res
            if (r["slack_reserved"] is not None and r["slack_reserved"] < r["slack_main"])
            else v_main
        )
        print(
            f"{r['rail']:<40}{r['reserve']:<24}{r['days_main']:>8}{r['slack_main']:>8}"
    ...
    for arr in (120_000, 500_000, 1_800_000):
        at_risk = arr * 0.10
        print(
            f"  ARR ${arr:>9,} | 10%/180d reserve on an MoR rail -> ${at_risk:>9,.0f} exposed; "
    ...
    for itp in INTERPRETATIONS:
        ...
        print(
            f"  {itp['name']:<46} cash in at day {cash_day:>4} | deadline day {deadline:>4} "
