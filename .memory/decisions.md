...
## D-296 (2026-09-19) — إلغاءُ شرط الخطوط السبعة نهائياً: السوقُ وحده هو الشرط (توجيه المالك)

**السياق (توجيه المالك — أمرٌ صريح):** «قم بحذف شرط السبعة نهائيا الذي يهم هو مايباع
وعليه طلب فقط» (2026-09-19) — امتدادٌ لتوجيه 2026-09-19 السابق (نطاقُ البحث غيرُ مقيَّدٍ
بالخطوط السبعة إطلاقاً). المالكُ يأمر بحذف الشرط من الحوكمة نفسها، لا من البحث وحده.

**القرار:** أُلغي الحصرُ العددي لخطوط الإيراد (D-273 §03/L9 + D-290 L6 كما كانا) نهائياً:
لا عددَ ثابت ولا قائمةَ مغلقة. الشرطُ الوحيد لخطٍّ مشروعٍ سوقيٌّ: **بيعٌ خارجيٌّ موثّق**
(سجلّ MF: سندُ `S-###` بسعرٍ معلنٍ أو دفعةٍ مثبتة) **وطلبٌ قائم** (مشترٍ محدَّد بمشكلةٍ
مدفوعة). الإضافةُ إلى `OFFER_CATALOG.json` بقرارِ باحثٍ/مالكٍ مكتوبٍ يستشهد بالسند.

**ما تغيّر (القانون الحيّ والفوارض فقط):**
- `check_agent_reliability_constitution.py` §7: حُذف `CANONICAL_OFFER_IDS` والحصرُ؛ تحرس فرادةَ
  المعرّفات وانضباطَ الحالات وأدلةَ الدفع (وكتالوجٌ فارغ = انتهاك).
- `check_commercial_offers_parity.py`: `OUTSIDE_CANONICAL_SEVEN` ← `OUTSIDE_CATALOG` في
  البوّابة ووثائق العروض الستّ واختباريهما.
- `DEEP_TECH_CONSTITUTION.md` §03/§05/§06: السوقُ وحده هو الشرط؛ الجدولُ محفظةٌ حالية لا حصر.
- دستور D-290 (القانون L6 + §6 + §7) و`CLAUDE.md` (§0.25 L9 + §0.29 — بلا تغييرٍ في عدد الأسطر)
  والوثيقة الاستراتيجية (§264 + R15) وخريطةُ الإيراد والكتالوج (`amended_by`).
- وثائقُ الحالة الحيّة: `as-of` + ملحق D-296؛ وبرومبت MF (v1.1: البحثُ سوقيٌّ خالص).

**ما لم يتغيّر عمداً (سجلٌّ تاريخي):** نصوصُ القرارات السابقة في `decisions.md` ·
سجلّاتُ الدفعات (`*_truth.md` وهوامشُ «ولا ثامن» فيها) · أوراقُ البحث المؤرَّخة ·
`docs/archive/` · `GATE_C = ABSENT` · K1–K4 · المحرّمات التسعة · L1–L12 عدا الحصر.

**مقياس القبول:** `make gates` أخضر (بما فيها البوّابتان المعدَّلتان واختباراهما) ·
`CLAUDE.md` ‏≤ 1262 سطراً · صفرُ إشارةٍ حيّةٍ إلى حصرٍ عددي خارج سياق الإلغاء.

## D-295 (2026-09-15) — «الاعتمادُ المجانيّ ثمنُ المسار لا خصمٌ من الفاتورة»: دفعة CPN-12

**ما وقع.** `UCL` (الدفعة السابقة) صنّفت `M3 cbam_pin.py` **أعلى حركةٍ مقيسة** في السجلّ
(11,000.00 €/يوم · 3.00 يوم جهد) — والملفُّ **لم يكن على القرص**. وبقيت أعلى حركةٍ
مُرتَّبة تسعةَ أيام بلا أداة، بينما أداتان على القرص تتناقضان بنسبة **39×** في المعامل
نفسه: `research/fx-hard-currency/cbam_value.py` يستعمل `FACTOR = {2026: 0.025}` مضروباً في
الفجوة، و`studies/cbam-markup-gradient-002/markup_gradient.py` يستعمل
`CBAM_FACTOR = {2026: 0.975}` مضروباً في الوفر. ودراسة 002 نصّت على «بيع MRV» (002-07)
بلا قياس — لأنها بُنيت على **عمودٍ واحد**، والملحقُ 5 من IR 2025/2620 ينشر **عمودين**.

**القرار.** فُحصت المصادرُ الأوّلية قبل كتابة سطر: ملفّا المفوضية الرسميّان (القيمُ
الافتراضية — النسخة 2 · المراجع — النسخة 1)، دليلُ DG CLIMA (جدول 8)، أسئلةُ المفوضية
وأجوبتها، رأسُ الملحق I من IR 2026/1740. ثمّ بُنيت `shared/research/cbam_pin.py` بدبوسٍ
حرفيّ: **69** صفاً جزائرياً + **56** رمزاً مرجعياً × العمودَين A/B + جدولُ العلاوة +
المعاملُ + CSCF + سعرُ الشهادة. والنتيجةُ المحورية **رسمُ المسار**:

```
path_toll = f × CSCF × (BM_B − BM_A)      ·      «الفجوةُ الساذجة» − «الوفرُ الحقيقيّ» = الرسمُ بالضبط
```

مُختبَرةٌ على شبكةٍ لـ **43** رمزاً × 3 سنوات = **129 تحقّقاً، صفرُ استثناء**. وفي **90**
زوجاً مدبوساً **B ≥ A دائماً** (73 · 17 تعادلاً · **صفرٌ** فيها A > B).

**ما سُحِب، وكيف.** عملاً بعرف RCL: **السحبُ يُسجَّل في سجلِّه ولا يُنشَر إلى مصادر
الأرقام**. ⛔ لم يُحرَّر `cbam_value.py` ولا `markup_gradient.py` ولا `ucl_model.py` —
حرفاً واحداً. والحارسُ
...
والسحبُ مُقيَّد: 002-07 **مقلوبٌ للأسمدة** (يوريا −15.33 €/t) · **مضيَّقٌ للصلب**
(8.47×) · **مؤجَّلٌ للإسمنت** إلى 2027 — ⛔ ولا يُستبدل بحكمٍ معاكسٍ عامّ: `7201`
والهيدروجين **غيرُ مشروطَين من 2026**.

**ثلاثُ قواعدَ منهجيةٍ أُضيفت.**

1. **⛔ لا تُرتَّب حركةٌ على نطاقٍ يورويّ ما لم يكن في المستودع ملفُّ قياسٍ مُودَع
   ببصمةٍ محسوبةٍ يحمل ذلك النطاق.** مسحٌ كاملٌ للمستودع أثبت أنّ بصماتِ `UCL` الستَّ
   المُورَّثة (`SFD`·`ERD`·`PTD`·`PID`·`ABR`·`CPD`) تظهر في **ملفَّين فحسب** — وثيقتِها
   وأداتِها — أي في الدفعة نفسها. فنطاقُ M3 (6–60 ألف €/day) كان **غيرَ قابلٍ للتدقيق**،
   ولهذا اشتُقَّت في CPN **الآليةُ** من مصدرٍ أوّليّ وتُرك النطاقُ اليورويّ **بلا ادّعاء**
   (`revenue_claim = NONE`). مُثبَّتٌ باختبارٍ لا بملاحظة.
2. **الافتراضُ يُوسَم في المُخرَج لا في النيّة.** `CANDIDATE_SEE` هو المُدخَلُ الوحيدُ في
   الدفعة الذي ليس من مصدرٍ أوّليّ؛ فوُسِم `ASSUMED_SEE (grade [A])` في **كلّ** مخرَج،
   ومُنعت العتباتُ (`crossover_see`) و`unconditional_year` من قراءته **باختبارٍ يمسح
   أجسامَ الدوال نصيّاً**. المُنتَجُ هو ما لا يحتاج افتراضاً.
3. **الغيابُ أصنافٌ لا صنفٌ واحد.** 26 رمزاً غيرُ قابلةٍ للترتيب بأربعةِ أسبابٍ
   **منطوقة**: `NO_COUNTRY_VALUE_USE_OTHER_COUNTRIES_TABLE` (7) ·
   `BENCHMARKS_NOT_EXTRACTED` (17) · `BENCHMARK_ROUTE_AMBIGUOUS_…` (1) ·
   `BENCHMARK_ROUTE_KEYS_UNPAIRED` (1). وتجميعُها في سببٍ واحدٍ كان سيُخفي أنّ
   الألومنيوم (K5) غائبٌ لسببٍ **قابلٍ للإغلاق ببطاقة**، بينما `7202` غائبٌ لسببٍ يحتاج
   جدولاً آخر.

**الدحضُ على أنفسنا مرّتين.** (1) الادّعاءُ الأوّليّ أنّ «كلَّ رمزٍ حاملِ رسمٍ مشروطٌ في
2026» — دُحض بـ `7201` (رسمٌ 8.88 €/t مقابل إعفاءٍ 21.61 €/t)، فسُقِط وصيغ من جديد:
**36** رمزاً مشروطاً و**7** غيرَ مشروطة. (2) الادّعاءُ الأوّليّ أنّ «العمودَ B أكبرُ
دائماً» — دُحض بـ **17 تعادلاً**، فسُقِط التعميمُ وبقي التوزيع. **الفحصُ المليمتريّ هو
ما يمنع التعميمَ من أوّل استثناء.**

**ما لم يتغيّر (⛔ صريح).** `GATE_C = ABSENT` — صفرُ عقد، صفرُ فاتورة، صفرُ دفعة،
**صفرُ عرضٍ مُرسَل**. **لا خطَّ ثامناً** في الكتالوج (بقي **سبعة**) — الدفعةُ تقيس ما
يجوز أن يدخل عرضاً، ⛔ لا تفتح عرضاً (D-290 L8). **لا رقمَ إيراد** ولا تسعير. **ليست
رأياً قانونياً**: ملحقُ IR 2025/2620 الحرفيّ لم يُستخرج (S4 درجة ب · **K3**)، وCSCF من
ID 2026/1862 **لم يُقرأ نصُّه** (درجة ب · **K2**) ⇒ **كلُّ رقمٍ باليورو `CONDITIONAL`**
وحساسيتُه محسوبة (انتشارُ عتبة الإسمنت 0.06494 t/t).

**المُخرَجات.** `shared/research/cbam_pin.py` (stdlib فقط) · `tests/shared/test_cpn_cbam_pin.py`
(**95** اختباراً، براهينُ سلبيةٌ لـ K1–K5 كلّها) · `scripts/research/measure_cbam_pin.py`
(بوّابةُ انحراف `--check` ✅) · `docs/research/CPN_MEASUREMENTS.json` (بصمةٌ محسوبة
`e95a8213…4dbcd4`) · `docs/research/HARD_CURRENCY_NEW_KNOWLEDGE_CPN.md` ·
`docs/research/README_NEW_KNOWLEDGE.md` §10 · `studies/cbam-default-pin-004/` (README ·
CLAIMS بـ **22** ادّعاءً · SOURCES بـ **13** مصدراً) · إلحاقٌ مؤرَّخ §7 في وثيقة UCL ·
`A22`–`A24` + `S74`–`S82` في سجلّ الادعاءات الحدّية.

**الجملةُ التي تُختصر بها الدفعة.** في نظامٍ بعمودٍ واحد تكون الفجوةُ هي الوفر. في
نظامٍ بعمودَين **يختار مسارُ الإعلان أيَّهما يُستعمل**، والعمودُ الأسخى مقترنٌ بالكسل —
فالكسلُ مدعومٌ والاجتهادُ مُسعَّر. ولهذا ليست النتيجةُ «لا تقيس» بل: **قِس، ثم قارن
قياسَك بعتبةٍ لا بفاتورة.**

---

## D-294 (2026-09-12) — «بوّابةٌ لا تُشغَّلُ محليّاً ليست بوّابةً أُجيزت»: روف/مايبي في دفعة AHW

**ما وقع.** أُغلقتِ الدفعةُ الرابعة (D-292) وملحَقُها (D-293) على بيانٍ بـ«كلُّ البوّاباتِ خضراء»، وكان
البيانُ صحيحاً **في مجموعةِ البوّاباتِ التي عندنا** (`scripts/fitness/*` + guardrails) وخاطئاً **بالنسبةِ
لبوّاباتِ CI** التي لا تتوفّر أدواتُها في بيئةِ التحقّق المحلية: `ruff==0.14.0` (المُثبَّتةُ عمداً في
`ci.yml` بعد D-184) و`mypy==1.8.0` على `shared/`. أوّلُ تشغيلٍ فعليٍّ لهما على هذا الفرع — في CI — أعطى
**13 خطأَ روف** (تنظيمُ استيرادات، `noqa: E402` زائدٌ لأنّ القاعدةَ مُستثناةٌ أصلاً في `pyproject.toml`،
استيراداتٌ غيرُ مستعمَلة، `UP017`، و`RUF022` لائحةُ `__all__` التي كان يجب أن تبقى مُرتَّبةً ترتيباً
isort‑يًّا) و**7 أخطاءِ مايبي** في `shared/research/assurance_window.py`: إعادةُ `Any` من عمليّاتِ أسٍّ
(`ratio**eps`)، `float(str(...))` التفافيٌّ في `summarize`، تمريرُ `None` في `curve_family`، و
`# type: ignore[arg-type]` **بات غيرُ مستعمَل** (`warn_unused_ignores = true`) أي أنّ الكودَ كان يخفي
حقيقةَ أنّ قاموساً غيرَ مُعنونٍ كان يُمرَّرُ بـ`**`.

**القرار (إصلاحٌ لا توسيعُ استثناءٍ).** لم تُزَد أيُّ سطرٍ في `per-file-ignores` — وهي قائمةٌ **تتقلّصُ
ولا تتّسع** بنصِّ تعليقِها — ولم تُخفَّف `mypy.ini`. بدلاً من ذلك:
1. `math.pow` بدلَ `**` حيث يُصرَّحُ نوعُ الإرجاع (نفسُ القيمة، نوعٌ مُعلَن)؛
2. `rates: dict[str, float] = {}` وإزالةُ `type: ignore` التي لا تحتاجُها؛
3. `curve_family` يُرجِعُ `dict[str, float | None]` **ويمرّرُ `None` صراحةً** بدلَ `or 0.0` — لأنّ
   «صفرُ أثر» ادّعاءٌ و«لا يُحسبُ الآن» أمانة (قاعدة D-212 نفسها)؛
4. `most_volatile_suite` يُحسَبُ بـ`max(drifts, key=drift_velocity)` بدلَ قرّاءةِ الجدولِ بنوعِ `str`؛
5. `__all__` أعيدَ ترتيبُها (123 اسماً، المجموعةُ ذاتُها بلا زيادةٍ ولا نقصان — مُتحقَّقٌ بالفرز)، و
   `# noqa: E402` حُذِف حيث هو زائد.
وبعدَها: `ruff check .` نظيف، `ruff format --check .` نظيف، `mypy` نظيفٌ على الوحدة، وملفُّ القياس
**لم يتغيّر** (`measure_assurance_window.py --check` يمرّ) — أي أنّ الإصلاحَ شكليٌّ محضٌ ولم يلمسْ رقماً،
وهو معيارُ القبولِ هنا.

**القاعدةُ المُستخلَصةُ للمراتِ القادمة (تُطبَّقُ على أيِّ دفعةِ بحثٍ تالية).** إن لم تكن الأداةُ متوفّرةً
محليَّاً فالحلُّ **تنصيبُها بقيدِ CI نفسِه** (`pip install ruff==0.14.0 mypy==1.8.0` — ثلاثُ ثوانٍ) أو
تسميةُ العجزِ صراحةً في `not_run_ar`؛ ⛔ لا يُقالُ «كلُّ البوّاباتِ خضراء» بل «البوّاباتُ العشرُ المذكورةُ
أدناه خضراء، وروف/مايبي مؤجَّلتانِ إلى CI» — وهذا فرقٌ من جنسِ الفرقِ بين «ادّعاءٌ صحيح» و«ادّعاءٌ لم
يُفتَح». ومعه أُضيفت إلى `verification.commands` في حزمةِ القبولِ سطورُ روف/فورمات/مايبي، لا بوصفها
زينةً بل بوصفها **ما كان ينقصُ البيانَ السابق**.

**الحدود.** لا يغيّرُ هذا شيئاً في الكتالوجِ ولا في الحالةِ التجارية: `GATE_C = ABSENT`، لا خطَّ ثامناً،
لا رقمَ إيراد. وهو لا يدّعي أنّ CI كان سيُقبَلُ لولا ذلك: الإخفاقاتُ العشرةُ القائمةُ في
`tests/shared/test_messaging.py` سابقةٌ للفرعِ ولا تمسُّها أيُّ ملفٍّ في هذا الطلب.
