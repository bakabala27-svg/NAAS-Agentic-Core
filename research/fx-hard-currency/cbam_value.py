...
MARKUP = {2026: .10, 2027: .20, 2028: .30, 2029: .30, 2030: .30}
# CBAM factor = 1 - free allocation share. EU ETS free allowance phase-out path.
FACTOR = {2026: .025, 2027: .05, 2028: .10, 2029: .225, 2030: .485}
...
def main():
    print(f"{'sector':28}{'t/yr':>11}{'gap tCO2/t':>12}"
          + "".join(f"{y:>12}" for y in FACTOR))
    totals = {y: 0.0 for y in FACTOR}
    for name, (tonnes, dflt, actual) in SECTORS.items():
        row = ""
        for y in FACTOR:
            markup = .01 * (y - 2025) if name == FERT else MARKUP[y]
            gap = dflt * (1 + markup) - actual
            value = tonnes * gap * FACTOR[y] * CERT / 1e6  # EUR millions
            totals[y] += value
            row += f"{value:12.1f}"
        print(f"{name:28}{tonnes:11,}{dflt - actual:12.2f}{row}")
    print(f"{'TOTAL EUR millions/yr':28}{'':11}{'':12}"
          + "".join(f"{totals[y]:12.1f}" for y in FACTOR))

    print("\nCapture test for one independent researcher:")
    for y in (2026, 2028, 2030):
        pool = totals[y] * 1e6
        for share in (.0005, .002, .01):
